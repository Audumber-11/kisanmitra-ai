...
import { askOpenRouter, getAIStatus } from "@/lib/ai-service";
...
export default function VoicePage() {
  ...
  const [aiStatus, setAiStatus] = useState<{ available: boolean; model: string | null }>({
    available: false,
    model: null,
  });
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      // Read AI status from env (set at build time by Next.js)
  ...
  const processQuery = useCallback(async (query: string) => {
    ...
    try {
      // Use OpenRouter AI directly from browser (with KB fallback)
      // supported languages: hindi, marathi; for others, fall back to KB
      const supportedLangs = ["hindi", "marathi"];
      const lang = supportedLangs.includes(selectedLanguage.api)
        ? (selectedLanguage.api as "hindi" | "marathi")
        : "hindi";

      const result = await askOpenRouter(query, lang);
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-primary/5 to-background py-8 px-4">
      <div className="container mx-auto max-w-3xl">
        ...
        {/* API Key Banner */}
        {!aiStatus.available && connectionState === "idle" && (
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="font-semibold text-blue-900 dark:text-blue-100">
                    Free AI mode available — add your OpenRouter key
                  </p>
                  <p className="text-sm text-blue-700 dark:text-blue-300 mt-1">
                    Without an API key, KisanMitra uses a built-in knowledge base for common farming questions.
                    Add a free OpenRouter key to unlock full AI chat for any farming question.
                  </p>
                  <div className="mt-3 flex flex-wrap gap-2">
                    ...
                    <span className="text-sm text-blue-600 dark:text-blue-400 self-center">
                      Then run{" "}
            ...
            <CardContent className="space-y-3 max-h-96 overflow-y-auto">
              {messages.map((msg, idx) => (
                ...
                >
                    ...
                    {msg.role === "user" ? "👨‍🌾 You" : `${msg.source === "ai" ? "🤖 AI" : "📚 Knowledge Base"} — KisanMitra AI`}
                  </p>
